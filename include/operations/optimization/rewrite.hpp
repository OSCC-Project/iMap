...
public:
  ...
  rewrite_impl(Ntk const& ntk, const rewrite_params& params, const ResynthesisFn& resynthesis_fn, const NodeCostFn& node_cost_fn)
    : _ntk(ntk),
  ...
  Ntk run()
  {
    ...
    _ntk.foreach_pi( [&]( auto const& n ) {
      map_old_to_new[n] = dest.create_pi();
    } );
...
private:
  ...
  uint32_t compute_total_cost( const Ntk& ntk)
  {
    ...
    ntk.foreach_gate( [&](auto const& n){
      total_cost += _node_cost_fn(ntk, n);
    });
    return total_cost;
  }
...
public:
  ...
  rewrite_online_impl(Ntk const& ntk,network_cuts_t& cuts, const rewrite_params& ps, const ResynthesisFn& resynthesis_fn, const NodeCostFn& node_cost_fn, cut_enumeration_params const& cut_ps)
    : _ntk(ntk),
      _resynthesis_fn(resynthesis_fn),
      _node_cost_fn(node_cost_fn),
      _params(ps),
...
template<typename Ntk = iFPGA_NAMESPACE::aig_network, typename ResynthesisFn = node_resynthesis<Ntk>, typename NodeCostFn = unit_cost<Ntk>> 
...
Ntk rewrite_online(Ntk const& ntk, rewrite_params const& params)
{
  ...
  const auto dest = detail::rewrite_online_impl<Ntk, ResynthesisFn, NodeCostFn>{ntk ,cut_network ,params ,resynthesis_fn ,cost_fn ,params.cut_enumeration_ps}.run();
  return dest;
}
