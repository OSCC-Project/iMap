...
namespace percy
{
    ...
    public:
        ...
        /* mockturtle style clause */
        synth_result solve(std::vector<uint32_t> const& clause, int cl)
        {
            auto lits = (int*)(const_cast<uint32_t*>(clause.data()));
            return solve(lits, lits + clause.size(), cl);
        }

        // propagate unit clauses
        void propaget_unit_clauses()
        {
            if( solver->qtail != solver->qhead)
            {
                int status = pabc::sat_solver_simplify(solver);
                assert(status);
